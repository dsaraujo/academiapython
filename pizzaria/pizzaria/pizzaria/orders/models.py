...
class Client(models.Model):
    ...
    def google_map_url(self):
        complete = self.full_address() + ', ' + self.district
        return u'http://maps.google.com/maps?q=%s' % complete.replace(' ', '+')
        
    full_address.short_description = u'address'
    
    @models.permalink
    def get_absolute_url(self):
        return('info-cli', (), {'pk': self.id})
...
class Order(models.Model):
    ...
    def __unicode__(self):
        ...
        return u'%s pizza%s to %s @ %s' % (num2eng.num2eng(c), 
